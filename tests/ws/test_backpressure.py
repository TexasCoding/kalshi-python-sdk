...
class TestMessageQueue:
    ...
    async def test_deque_maxlen_caps_memory_under_sustained_overflow(self) -> None:
        """Regression for #173: the underlying deque carries `maxlen=maxsize+1`
        as a defense-in-depth ceiling. Even under sustained overflow far past
        the configured ``maxsize``, the buffer cannot grow without bound —
        ``maxlen`` enforces the cap at the C level.
        """
        ...
        # qsize() reflects the real (capped) occupancy.
        assert q.qsize() == q._maxsize
